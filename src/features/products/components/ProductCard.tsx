  ...
  return (
    <Link to={`/products/${product.id}`} className="card card--product bg-cover bg-center relative block" style={{ backgroundImage: `url(${product.thumbnail})` }} aria-label={`View ${product.title}`}>
      ...
      <div className="relative flex h-full flex-col">
        ...
        <div className="products-card__actions mt-auto">
          {/* These buttons live inside a Link card.
              Stop the click here first so the action runs without the whole card navigating away. */}
